# Publicly available books

Hi, I'm listing down all the publicly available books along with their links, which I found really helpful.

## Statistics
 - [Think Stats](http://www.greenteapress.com/thinkstats/thinkstats.pdf)
 - [OpenIntro Statistics](https://www.openintro.org/stat/os4.php)

## Machine Learning
- [An Introduction to Statistical Learning](http://www-bcf.usc.edu/~gareth/ISL/ISLR%20Seventh%20Printing.pdf)
- [Bayesian Reasoning and Machine Learning](http://web4.cs.ucl.ac.uk/staff/D.Barber/textbook/091117.pdf)
- [The Elements of Statistical Learning](https://web.stanford.edu/~hastie/ElemStatLearn/printings/ESLII_print12.pdf)

## Deep Learning
- [Deep Learning](https://www.deeplearningbook.org/)
- [Neural Networks and Deep Learning](http://neuralnetworksanddeeplearning.com/index.html)
